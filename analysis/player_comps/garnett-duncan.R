...
### Entire NBA ###
### First five years for each player adding year column ###
first_five_info = ddply(both_info_salaries, "id", function(x) head(x[order(x$season, decreasing = FALSE) , ], 5))
first_five_info = ddply(both_first_five_info, "id", transform, year = seq_along(season))
first_five_info
...
b <- b + xlab("Year") + ylab("Rebs") + ggtitle("Rebs by season")
b
...
d <- ggplot(data = player_first_five_team, aes(x = year, y = percent_w, col = PLAYER))
d <- d + geom_point(size = 5)
d <- d + xlab("Year") + ylab("Team Wins") + ggtitle("Team Wins by season")
d
